"""
PyPSA StorageUnit CP vs C cycling experiment  (v2 — realistic hourly dispatch)
==============================================================================
PyPSA version:  1.3.0
...
DIFFERENCE FROM experiment.py (v1)
-----------------------------------
v1 uses snapshot_weightings = 912.5 h. At that weighting a single snapshot
represents ~38 days, so the optimiser charges the battery from 0 to full in a
single snapshot — mechanically correct but not a physically realistic dispatch
profile.

v2 uses snapshot_weightings = 1.0 h (each snapshot = one actual hour) and
scales the capital costs by SCALE = 1/912.5 so the investment economics are
equivalent. The result is a smooth bell-curve SOC profile: the battery charges
gradually through the day and discharges through the night. The boundary-
equation mechanism (the point of the experiment) is IDENTICAL between v1 and v2.
...
Verified v2 output: battery builds ~62 MW (Run A) / ~60 MW (Run B) with a
smooth bell-curve SOC cycle (0 → ~370 MWh in Run A, 0 → ~68 MWh peak within
period 2 in Run B).
...
-------------------------------------
See patch_spillage.py and README.md. If the installed PyPSA 1.3.0 uses
get_as_dense for inflow (as most release artefacts do), no patch is needed.
...
# v2 uses snapshot_weightings = 1.0 h instead of v1's 912.5 h. To keep the
# investment economics equivalent, capital costs are scaled down by the same
# factor (the ratio of the v1 weighting to the v2 weighting).
_COST_SCALE = 1.0 / 912.5
...
def build_network(
    ...
    # v2: 1 hour per snapshot (realistic hourly dispatch). Capital costs are
    # scaled by _COST_SCALE below to keep investment economics equivalent to
    # the v1 912.5 h weighting.
    ...
    n.add(
        ...
        p_nom_min=0.0,
        p_nom_max=500.0,
        capital_cost=200_000.0 * _COST_SCALE,   # £/MW (scaled for 1h weighting)
    ...
    n.add(
        ...
        capital_cost=1_500.0 * _COST_SCALE,     # £/MW (scaled for 1h weighting)
        marginal_cost=0.0,
